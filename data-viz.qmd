...
knitr::opts_chunk$set(
  ...
  warning = FALSE,     # Hide warnings from execution
  message = FALSE,     # Hide messages/loading text
  fig.width = 8,       # Default width for plots (inches)
  fig.height = 6,      # Default height for plots (inches)
  fig.format = "png",
  dpi = 300,
...
Shown with direct labels (preferable) and a y-axis
...
### Single Column Chart with Contrast color
...
colors <- c("Dark" = "#174a7c", "Light" = "#b0d0db", "Grey" = "#bcbec0")
...
### Case Study: Stacked vs Grouped Column Charts
...
**When to Use a Grouped Column Chart**

- When you have a lot of groups and want to better show off a group that isn't at the bottom your stacked columns

Both of the charts shown below have too many groups to be really effective as stacked **or** grouped column charts. 
...
A small multiples chart that shows individual graphs for either 1) the income group or 2) the income source is a better option in this case.

::: {.panel-tabset}

#### R

```{r small-multiples-column}
#| fig-width: 10
#| fig-height: 10
#| out-width: "85%"
#| fig-align: center
...
ggplot(income_composition) +
 ...
 theme(axis.text.y = element_text(family = "Avenir TPC", size = 10, margin = margin(r=10)),
  ...
  theme(
    ...
    axis.ticks.x = element_blank(),
    plot.margin = margin(t = 5.5, r = 5.5, b = 24, l = 5.5, unit = "pt")
  )
```

```{r small-multiples-column-2}
#| fig-width: 10
#| fig-height: 8
#| out-width: "85%"
#| fig-align: center
ggplot(income_composition) +
  geom_col(aes(x = series, y = value, fill = series, group = income_bracket), position = "dodge") + 
  scale_y_continuous(labels = scales::percent_format(),
                     breaks = seq(0,1,0.25),
                     expand = expansion(mult = c(0, 0.05))) +
  labs(
...
### Single column chart with contrasat color
